...
const friends = useMemo(() => {
        ...
        return friendList?.response.filter((friend) => friend?.userId !== currentUser?.id) || []; // Use the response from the friendList or an empty array if loading
  ...
  const filteredFriends = friends
        .filter(friend => friend.displayName.toLowerCase().includes(searchTerm.toLowerCase()))
        .sort((a, b) => {
            if (sortBy === "Tên (A-Z)") {
  ...
  const handleShare = async () => {
    ...
    const payload = {
      ...
      content: selectedMessage.content + (messageContent ? `\n\n${messageContent}` : ""),
    };
      ...
      <Modal.Body>
        ...
        {selectedMessage && (
          <div className="forwarded-message-preview mb-3 p-3 bg-light rounded">
            ...
            <p className="mb-0">
              {selectedMessage.content || selectedMessage.text || "(Không có nội dung)"}
            </p>
          </div>
        )}
        ...
        {/* Sắp xếp */}
        <div className="dropdown-container mb-3" onMouseLeave={() => setSortOpen(false)}>
          <div
            className="sort-box"
          ...
          {sortOpen && (
            ...
            >
              ...
              >
                {sortBy === "Tên (A-Z)" && <FaCheck style={{ marginRight: "5px" }} />}
                <span>Tên (A-Z)</span>
              </div>
              <div
                ...
                style={{ padding: "5px 10px", cursor: "pointer" }}
              >
                {sortBy === "Tên (Z-A)" && <FaCheck style={{ marginRight: "5px" }} />}
                <span>Tên (Z-A)</span>
              </div>
            </div>
          )}
        ...
        {/* Danh sách bạn bè */}
        <ListGroup className="conversation-list" style={{ maxHeight: "300px", overflowY: "auto" }}>
          {isLoadingFriends ? (
